...
(function () {
  ...
  function initMarquee() {
    document.querySelectorAll("[data-marquee]").forEach((el) => {
      ...
      // Hint compositor — keeps the marquee on its own GPU layer.
      inner.style.willChange = "transform";
      clone.style.willChange = "transform";
      let x = 0;
      // Cache layout reads. Re-measure only on resize, not every frame —
      // reading offsetWidth inside rAF forces a synchronous layout each tick
      // and burned ~3s of main-thread time per Lighthouse trace.
      let w = inner.offsetWidth;
      const ro = ("ResizeObserver" in window)
        ? new ResizeObserver(() => { w = inner.offsetWidth; })
        : null;
      if (ro) ro.observe(inner);
      else window.addEventListener("resize", () => { w = inner.offsetWidth; }, { passive: true });
      function tick() {
